...
class ProbDef(object):
    ...
    def compute_gradient(self, x):
        ...
        print("%s -> %s"%(RANK,samples.num_config))
        ...
        #opq_vals = sr.opq.mpi_eval_on_samples(samples, rbm, update_samples=True)
        opq_vals = sr.opq.eval_on_samples(samples, rbm, update_samples=False)
        opq_vals = [np.asarray(opv*samples.num_sample) for opv in opq_vals]
        for j in range(opq.num_ops):
            if RANK==0:
