...
from qstate.sampler.mpiutils import RANK, SIZE, COMM
...
class ProbDef(object):
    ...
    def compute_gradient(self, x):
        ...
        opq_vals = [0]*sr.opq.num_ops
        if self.num_vmc_run != SIZE:
            raise NotImplementedError('SIZE!=num_vim_run')
        for i in range(self.num_vmc_run):
            if i%SIZE==RANK:
                samples = vmc.generate_samples(rbm, num_sample=self.num_vmc_sample, seed=np.random.randint(0,999999)+i)

        if SIZE>1:
            print("%s -> %s"%(RANK,(np.array(samples.counts)!=0).sum()))
            dicts = COMM.gather(dict(zip(samples.config_inds,samples.counts)), root=0)
            if RANK==0:
                pre_dicts = dicts[:1]
                for d in dicts[1:]:
                    nd = len(d)
                    for id, count in d.items():
                        for dict_i in pre_dicts:
                            if dict_i is d: continue
                            tcount = dict_i.get(id,0)
                            if tcount != 0:
                                if len(dict_i)>nd:
                                    d[id]+=tcount
                                    dict_i[id] = 0
                    ...
                    pre_dicts.append(d)
                dicts = pre_dicts
                print(np.sum([np.sum(list(di.values())) for di in pre_dicts]))
            else:
                pre_dicts = None

            dicts = COMM.scatter(pre_dicts, root=0)
            # udpate counts
            samples.counts = [dicts.get(id,0) for id in samples.config_inds]
            print("@%s -> %s"%(RANK,(np.array(samples.counts)!=0).sum()))

        #rbm.thnn.render_theta(samples)
        res = sr.opq.eval_on_samples(samples, rbm, update_samples=False)
        for j,res_j in enumerate(res):
            opq_vals[j]+=res_j

        opq_vals = [np.asarray(res_li/self.num_vmc_run) for res_li in opq_vals]
        if SIZE!=1:
            for j in range(sr.opq.num_ops):
                if RANK==0:
                    res_list_g = np.empty((SIZE,)+opq_vals[j].shape, dtype=opq_vals[j].dtype)
                else:
                    res_list_g = None
                COMM.Gather(opq_vals[j], res_list_g, root=0)
                if RANK==0:
                    opq_vals[j] = res_list_g.sum(axis=0)
