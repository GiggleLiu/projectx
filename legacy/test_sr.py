...
class SRTest(object):
    ...
    def test_sr(self,fakevmc=False):
        ...
        self.rbm=random_rbm(nin=self.nsite,nhid=self.nsite*nfeature,group=group)
        #variable mask for a, b, W.
        #In this case, we do not change a in optimization procedure, instead, we set a to 0.
        ...
        optimizer=GradientDescent(wrt=self.rbm.get_variables(),fprime=sr.compute_gradient,step_rate=2e-2,momentum=0.)
