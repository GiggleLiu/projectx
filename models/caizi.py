...
class CaiZi(StateNN):
    ...
    '''
    def __init__(self, input_shape, num_features1=[12], num_features2=[], itype='float64',version='basic', eta=0.2):
        self.num_features1, self.num_features2 = num_features1, num_features2
        nsite=np.prod(input_shape)
        super(CaiZi, self).__init__()

        # create amplitude network
        net1 = ANN(layers=[functions.Reshape(input_shape,itype=itype,output_shape=input_shape)])
        for nfi, nfo in zip([np.prod(input_shape)]+num_features1, num_features1+[1]):
        ...
        # create sign network
        net2 = ANN(layers=[functions.Reshape(input_shape,itype=itype,output_shape=input_shape)])
        for nfi, nfo in zip([np.prod(input_shape)]+num_features2, num_features2+[1]):
