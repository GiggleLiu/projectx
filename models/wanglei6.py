...
class WangLei6(StateNN):
    ...
    def __init__(self, input_shape, nonlinear_list, powerlist=None, num_features=[4,4,4], eta0=0.2, eta1=0.2, NP=1, NC=1, K=None,\
        ...
        if powerlist is not None:
            nfo = len(powerlist)
            plnn = ParallelNN(input_shape = ishape, output_shape=(1,nfo,)+input_shape, itype=itype, axis=1)
            for power in powerlist:
                plnn.add_layer(functions.ConvProd, powers=power, boundary='P', strides=(1,)*D)
